...
$("#btnPatSearch").click(function(e) {
  e.preventDefault();
  var patientname = $("#PatientSearchID").val();
  ...
  if (patientname != null && patientname.trim() != "") {
    $.ajax({
      ...
      $("#patientcontact").val(item.Contact),
      $("#patientaddress").val(item.Address),
      $("#patientnic").val(item.NIC);
      $("#txtPatNIC").val(item.NIC);
      $("#ViewPatientID").val(item.ID)
      $("#ViewPatientName").val(item.Name)
      })},
      error: function(data) {
        console.log(data);
...
$("#btnSearchPatHistory").click(function(e) {
  e.preventDefault();
  var patientnic = $("#txtPatNIC").val();
  //$("#patientname").val(patientname);
  //var userID = sessionStorage.getItem("log_user_id");
  console.log(patientnic);
  if (patientnic != null) {
    $.ajax({
      url: "http://localhost:51053/api/patient/SearchPatientHistory",
      type: "POST",
      dataType: "json",
      contentType: "application/json",
      headers: {
      ...
      $("#ViewPatientID").val(item.ID),
      $("#ViewPatientName").val(item.Name)
      $("#ViewPatientSymptoms").val(item.Symptoms),
      $("#ViewPatientDiagnosis").val(item.Diagnosis),
      $("#ViewPatientChanges").val(item.Changes),
      $("#ViewPatientPrescriptions").val(item.Prescription)
      $("#ViewPatientRemarks").val(item.Remarks);
      var dm = new Date(item.DateModified).toLocaleString();
      $("#ViewLastModified").val(dm);
      })},
      error: function(data) {
        console.log(data);
        toastr.error(data.responseJSON.Message, "Server Error!");
    ...
    toastr.error("Please Enter A Patient NIC.", "Invalid Details!");
  }
});


$("#btnUpdatePatientHistory").click(function(e) {
  e.preventDefault();
  var PatID = $("#ViewPatientID").val();
  var Symp= $("#ViewPatientSymptoms").val();
  var Diag = $("#ViewPatientDiagnosis").val();
  var Chg = $("#ViewPatientChanges").val();
  var Remarks  = $("#ViewPatientRemarks").val();
  var pres = $("#ViewPatientPrescriptions").val();
  var dateMod = getNowDate();
  if (PatID!=null) {
    $.ajax({
      url: "http://localhost:51053/api/patient/UpdatePatientHistory",
      type: "POST",
      dataType: "json",
      contentType: "application/json",
      headers: {
      ...
      success: function(data) {
        console.log(data);
        toastr.success(data, "Patient History Updated!");
